...
Compose still reads `env_file: .env.<env>` at runtime, which means
the plaintext exists on disk between deploys. The encrypted form is
the source of truth for storage and transport. Wrapping compose
itself in `sops exec-env` (so secrets only exist in container memory)
...
The `subscriptions.hmac_keys` column is `jsonb` and not wrapped in
v1 — a JSON-aware codec is the v2 follow-up.
...
- Backend and agent's `DATABASE_URL` includes
  `?sslmode=verify-full&sslrootcert=/etc/goldilocks-tls/ca.crt`. The
  CA cert is mounted into each container read-only.
- The backup container uses `PGSSLMODE=verify-full` with the same
  pinned CA for `pg_dump`.
- The CA itself (`secrets/tls/ca.crt` + `ca.key`) is a 10-year
  self-signed root, generated by `scripts/init-tls.sh` via the
  `step` CLI inside the backup image.
...
Mutual TLS — postgres requiring client certs from backend / agent —
is the obvious v2 step (`hostssl ... clientcert=verify-full` in
`pg_hba.conf`). Documented in the plan.
...
- **Sealed secrets still materialize as plaintext at deploy time.**
  Compose's `env_file:` reads plaintext. Wrapping compose itself in
  `sops exec-env` keeps secrets purely in container memory; a
  follow-up.
- **No mTLS to Postgres.** Server-side `clientcert=verify-full` adds
  another layer of "the attacker needs more than just being on the
  compose net."
...
- **No column-encryption key rotation flow.** Currently a one-time
  ...
  pattern is clearer.
- **`subscriptions.hmac_keys` (jsonb) not yet wrapped** — a
  JSON-aware codec is the small v2 piece.
