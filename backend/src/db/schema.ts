...
//
// Admins (Morgan, Tillie) are listed by inbox_id in `admin_inboxes`. The
// `/v2/me` endpoint returns `isAdmin: true` if the caller's inbox_id is
// in that table — the iOS app uses this to switch between client UI and
// admin UI.
...
// Allowlist: inbox IDs in this table are recognised as admins by /v2/me.
// Seeded with Morgan + Tillie in migration 002 so dev works out of the box.
export const adminInboxes = pgTable('admin_inboxes', {
  inboxId: text('inbox_id').primaryKey(),
  // Display label so admin lists/audit logs are human-readable.
  name: text('name'),
  createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
});
