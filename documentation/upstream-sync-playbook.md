...
The project carries `-warn-long-function-bodies` / `-warn-long-expression-type-checking` at 500ms (advisory warnings since the posture restore — see the previous lesson). When one fires, there are three distinct causes, and you must diagnose before "fixing":
