...
public protocol ContactsWriterProtocol: Sendable {
    ...
    /// Admin contacts cannot be blocked.
    ...
    /// Syncs admin contacts with the authoritative backend list. Upserts
    /// contacts for each admin with `isAdminContact = true`, updates names,
    /// and removes contacts that are no longer admins (only if they have no
    /// organic conversation association).
...
final class ContactsWriter: ContactsWriterProtocol, @unchecked Sendable {
    ...
    func block(inboxId: String) async throws {
        let didChange: Bool = try await databaseWriter.write { db in
            ...
            guard !existing.isAdminContact else {
                Log.debug("block(inboxId:) skipped, admin contacts cannot be blocked")
                return false
            }
